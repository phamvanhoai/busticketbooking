...
<!-- Heading -->
        ...
        <div class="grid md:grid-cols-2 gap-6">
          ...
          <div class="space-y-2">
            ...
            <div class="flex gap-2 items-center">
              ...
              <input id="rowsDown" type="number" value="4" min="1" max="10"
                     class="w-16 border rounded px-2 py-1"/>
              <span>×</span>
              <input id="colsDown" type="number" value="5" min="1" max="10"
                     class="w-16 border rounded px-2 py-1"/>
            </div>
            <div class="flex gap-2 items-center">
              <label>Row Prefix:</label>
              <input id="prefixDown" type="text" value="A" maxlength="2"
                     class="w-16 border rounded px-2 py-1"/>
            </div>
            <div class="flex gap-2 items-center">
              <label>Aisles (cols):</label>
              <input id="aislesDown" type="text" value="2"
                     placeholder="e.g. 2,5"
                     class="flex-1 border rounded px-2 py-1"/>
            </div>
            <button type="button" onclick="regenTable('down')"
                    class="px-4 py-2 bg-[#EF5222] text-white rounded hover:bg-opacity-90">
              Apply Downstairs
            </button>
          </div>
          <!-- Up floor -->
          <div class="space-y-2">
            ...
            <div class="flex gap-2 items-center">
              <label>Rows×Cols:</label>
              <input id="rowsUp" type="number" value="3" min="1" max="10"
                     class="w-16 border rounded px-2 py-1"/>
              <span>×</span>
              <input id="colsUp" type="number" value="4" min="1" max="10"
                     class="w-16 border rounded px-2 py-1"/>
            </div>
            <div class="flex gap-2 items-center">
              <label>Row Prefix:</label>
              <input id="prefixUp" type="text" value="B" maxlength="2"
                     class="w-16 border rounded px-2 py-1"/>
            </div>
            <div class="flex gap-2 items-center">
              <label>Aisles (cols):</label>
              <input id="aislesUp" type="text" value="1"
                     placeholder="e.g. 1,3"
                     class="flex-1 border rounded px-2 py-1"/>
            </div>
            <button type="button" onclick="regenTable('up')"
                    class="px-4 py-2 bg-[#EF5222] text-white rounded hover:bg-opacity-90">
              Apply Up floor
            </button>
          </div>
  ...
  <script>
    ...
    function regenTable(zone) {
      ...
      const aisles = document.getElementById('aisles' + capitalize(zone)).value
      ...
      for (let c=1;c<=cols;c++){
        ...
        if (aisles.includes(c)) {
          headRow.appendChild(cell('th','',true,'border-none w-6')); 
        }
      }
      ...
      for(let r=1;r<=rows;r++){
        ...
        for(let c=1;c<=cols;c++){
          ...
          tr.appendChild(td);
          if (aisles.includes(c)) {
            tr.appendChild(cell('td','',false,'border-none w-6 bg-gray-100'));
          }
        }
    ...
    function cell(tag,text,isHdr,extra=''){
      const el = document.createElement(tag);
      el.className = `${isHdr
        ? 'border p-2 bg-gray-100 font-medium'
        : 'border p-2 cursor-pointer hover:bg-gray-50'} ${extra}`;
      el.innerText = text;
      return el;
    }
    ...
    function toggleSeat(zone,el){
      ...
      if(i>=0){
        arr.splice(i,1); el.innerText=''; el.classList.remove('bg-[#EF5222]','text-white');
      } else {
        const code = prompt('Seat code (eg '+document.getElementById('prefix'+capitalize(zone)).value+r+'):' );
        if(!code) return;
        arr.push({r,c,code});
        el.innerText=code; el.classList.add('bg-[#EF5222]','text-white');
      }
    }
